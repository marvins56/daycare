  ...
  async (req, res) => {
    ...
    try {
      ...
      const babysitter = await Babysitter.create({
        ...
        nationalId: req.body.nationalId,
        dateOfBirth: req.body.dateOfBirth,
        nextOfKinName: req.body.nextOfKin.name,
        nextOfKinPhone: req.body.nextOfKin.phoneNumber,
        userId: userId
      });
...
//   }
// });
// In routes/babysitters.js, modify the GET responses:
router.get('/', protect, async (req, res) => {
  try {
    const babysitters = await Babysitter.findAll({
      include: [{ model: User, as: 'user', attributes: ['firstName', 'lastName', 'email'] }]
    ...
    const formattedBabysitters = babysitters.map(babysitter => ({
      _id: babysitter.id,
      ...
      nextOfKin: {
        name: babysitter.nextOfKinName,
        phoneNumber: babysitter.nextOfKinPhone
      },
      user: babysitter.user ? {
        _id: babysitter.user.id,
        firstName: babysitter.user.firstName,
        lastName: babysitter.user.lastName,
        email: babysitter.user.email
      } : null,
      createdAt: babysitter.createdAt
    }));
    
    res.json(formattedBabysitters);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
  }
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      let babysitter = await Babysitter.findByPk(req.params.id);
      ...
      if (req.body.nationalId !== babysitter.nationalId) {
        ...
        if (existingBabysitter && existingBabysitter.id !== parseInt(req.params.id)) {
          return res.status(400).json({ msg: 'Babysitter with this National ID already exists' });
        }
      }
      ...
      // Update associated user if exists
      if (babysitter.userId) {
        const user = await User.findByPk(babysitter.userId);
        if (user) {
          await user.update({
            firstName: req.body.firstName,
            lastName: req.body.lastName,
      ...
      // Reload babysitter to get updated data
      await babysitter.reload();
...
router.delete('/:id', [protect, authorize('manager')], async (req, res) => {
  try {
    ...
    // Delete associated user if exists
    if (babysitter.userId) {
      await User.destroy({ where: { id: babysitter.userId } });
    }
